...
//	int buttonFlag = 0;
  ...
  while (1)
  {
	  for(int i=0;i<7;i++)
	  {
//		  buttonState[i] = HAL_GPIO_ReadPin(BTN_GPIO_Port,BTN0_Pin);
		  buttonState[i] = HAL_GPIO_ReadPin(BTN_GPIO_Port,BTN0_Pin<<i);
		  if(buttonState[i] != buttonStateOld[i])   //버턴의 상태가 변경시
		  {
			if(buttonState[i])   //버턴이 On일때
			{
//				HAL_GPIO_TogglePin(LED_GPIO_Port,LED0_Pin);
				HAL_GPIO_TogglePin(LED_GPIO_Port,LED0_Pin<<i);
	/*
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : LED6_Pin LED0_Pin LED1_Pin LED2_Pin
